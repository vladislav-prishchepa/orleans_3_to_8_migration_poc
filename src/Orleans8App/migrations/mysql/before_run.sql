...
(
    'ReadFromStorageKey',
    'SELECT
        CAST(PayloadJson AS BINARY) AS PayloadBinary,
        UTC_TIMESTAMP(),
        Version
    FROM
        OrleansStorage
...
(
    'WriteToStorageKey','
    call WriteToStorage(@GrainIdHash, @GrainIdN0, @GrainIdN1, @GrainTypeHash, @GrainTypeString, @GrainIdExtensionString, @ServiceId, @GrainStateVersion, CAST(@PayloadBinary AS CHAR));'
)
ON DUPLICATE KEY UPDATE
    QueryText = VALUES(QueryText);
