...
(
    'WriteToStorageKey','
        ...
        select * from WriteToStorage(@GrainIdHash, @GrainIdN0, @GrainIdN1, @GrainTypeHash, @GrainTypeString, @GrainIdExtensionString, @ServiceId, @GrainStateVersion, convert_from(@PayloadBinary, ''UTF8'')::jsonb);
')
ON CONFLICT (QueryKey) DO UPDATE
    SET QueryText = EXCLUDED.QueryText;
...
(
    'ReadFromStorageKey','
    SELECT
        PayloadJson::text::bytea AS PayloadBinary,
        (now() at time zone ''utc''),
        Version
    FROM
        OrleansStorage
